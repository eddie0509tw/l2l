...
class MiniImagenet(Dataset):
    ...
    **Example**
    ...
    def __init__(
        ...
        self._bookkeeping_path = os.path.join(self.root, 'mini-imagenet-bookkeeping-' + self.image_set + '.pkl')

        self.db = self._get_db()
    
    def _get_db(self):
        data = self._loaddata()
        imgs = torch.from_numpy(data["image_data"]).permute(0, 3, 1, 2).float()
        labels = np.ones(len(imgs))

        # TODO Remove index_classes from here
        class_idx = index_classes(data['class_dict'].keys())
        for class_name, idxs in data['class_dict'].items():
            for idx in idxs:
                labels[idx] = class_idx[class_name]
        return {'imgs': imgs, 'labels': labels,}
        
    def __getitem__(self, idx):
        data = self.db['imgs'][idx]
        label = int(self.db['labels'][idx])
        if self.transform:
    ...
    def _check_exists(self):
        return os.path.exists(os.path.join(self.root, 'mini-imagenet-cache-' + self.image_set + '.pkl'))
    ...
    def _loaddata(self):
        ...
        pickle_file = os.path.join(self.root, 'mini-imagenet-cache-' + self.image_set + '.pkl')
        try:
            if not self._check_exists() and self._download:
                print('Downloading mini-ImageNet --', self.image_set)
            ...
            try:
                if not self._check_exists() and self._download:
                    print('Downloading mini-ImageNet --', self.image_set)
                    download_pkl(google_drive_file_id, self.root, self.image_set)
                with open(pickle_file, 'rb') as f:
                    data = pickle.load(f)
            except pickle.UnpicklingError:
                if not self._check_exists() and self._download:
                    print('Download failed. Re-trying mini-ImageNet --', self.image_set)
                    download_file(dropbox_file_link, pickle_file)
                with open(pickle_file, 'rb') as f:
                    data = pickle.load(f)
        return data
